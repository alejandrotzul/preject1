...
function checkTableName($shortTName, $type=false)
{
	...
	if ("asignacionanalista" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("insumos" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("pac" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("requisiciones" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("tipodecompra" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	return false;
}
